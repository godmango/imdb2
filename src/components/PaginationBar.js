...
const PaginationBar = ({ pageNum, setPageNum, totalPageNum }) => {
  ...
  return (
    <Pagination className="mt-3">
      ...
      {pageNum > 3 && <Pagination.Ellipsis disabled />}
      {pageNum > 2 && (
        <Pagination.Item onClick={() => handleClickOnPage(pageNum - 1)}>
          {pageNum - 1}
        </Pagination.Item>
      )}
      ...
      {pageNum > 0 && pageNum !== totalPageNum && (
        <Pagination.Item onClick={() => handleClickOnPage(pageNum + 1)}>
          {pageNum + 1}
        </Pagination.Item>
      )}
      {pageNum < totalPageNum - 1 && <Pagination.Ellipsis disabled />}
      {totalPageNum > 1 && (
        <Pagination.Item
